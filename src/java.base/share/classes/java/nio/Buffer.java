...
public abstract class Buffer {
    ...
    // ResourceScope lock used by Buffers backed by MemorySegment
    AutoCloseable scopeLock;
    ...
    static {
        ...
        SharedSecrets.setJavaNioAccess(
            new JavaNioAccess() {
                ...
                @Override
                public void lockBufferScope(Buffer buffer) {
                    if (buffer != null) {
                        ScopedMemoryAccess.Scope scope = buffer.scope();
                        if (scope != null) {
                            /*
                             TODO:
                              Update test/jdk/java/foreign/TestIOWithByteBuffer.java
                              to check for assertion below as it's currently failing
                              due to check which tries to access scope illegally
                             */
                            // assert buffer.scopeLock == null;
                            buffer.scopeLock = scope.lockScope();
                        }
                    }
                }

                /** Unlock scope if set. Otherwise, do nothing */
                @Override
                public void unlockBufferScope(Buffer buffer) {
                    if (buffer != null) {
                        var lock = buffer.scopeLock;
                        ScopedMemoryAccess.Scope scope = buffer.scope();
                        if (scope != null && lock != null) {
                            scope.unlockScope(lock);
                            try {
                                lock.close();
                                lock = null;
                            } catch (Exception ex) {
